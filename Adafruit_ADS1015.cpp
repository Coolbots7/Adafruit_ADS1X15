...
void Adafruit_ADS1015::writeRegister(uint8_t reg, uint16_t value) {
  _i2c->beginTransmission(m_i2cAddress);
  _i2c->write(reg);
...
uint16_t Adafruit_ADS1015::readRegister(uint8_t reg) {
  ...
  _i2c->requestFrom(m_i2cAddress, (uint8_t)2);
...
*/
/**************************************************************************/
void Adafruit_ADS1015::begin(TwoWire *theWire) {
  _i2c = theWire;
  _i2c->begin();
}
...
uint16_t Adafruit_ADS1015::readADC_SingleEnded(uint8_t channel) {
  ...
  return  (uint16_t)(readRegister(ADS1015_REG_POINTER_CONVERT) >> m_bitShift);  
}
...
int16_t Adafruit_ADS1015::readADC_Differential_0_1() {
  ...
  // Read the conversion results
  uint16_t res = (uint16_t)(readRegister(ADS1015_REG_POINTER_CONVERT) >> m_bitShift);
  if (m_bitShift == 0)
  {
    return (int16_t)res;
  }
